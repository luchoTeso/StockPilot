...
**Estado:** El hallazgo E1 ya se corrigió y subió a `main` (commit `c6ae00d`). Fases 0, 1, 2, 2b, 3 y 5 implementadas y verificadas en `feature/unificacion-motor-riesgo` (en rama local, pendiente de que el usuario las pruebe manualmente antes de subir a `main`); la exclusión de Promociones de la decisión 6 también quedó lista (ver 8.7). Falta la Fase 4 (unificación de `Alert.js`) y la consolidación opcional de la Fase 6. Ver sección 8 para las decisiones finales y las fases actualizadas.
