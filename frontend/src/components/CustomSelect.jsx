...
const CustomSelect = ({ value, onChange, options, placeholder, className }) => {
  ...
  const dropdownPortal = isOpen ? createPortal(
    <div 
      ...
      className="fixed z-[9999] bg-white border border-slate-100 rounded-2xl shadow-lg overflow-hidden animate-scale-in py-2 max-h-60 overflow-y-auto scrollbar-premium"
    >
      {options.map((opt) => (
        <div 
          key={opt.value}
          ...
          className={`px-4 py-3 text-sm font-bold cursor-pointer transition-colors hover:bg-azul/10 hover:text-azul focus:bg-azul/10 focus:text-azul outline-none ${value === String(opt.value) || value === opt.value ? 'bg-azul/10 text-azul' : 'text-tinta-2'}`}
        >
          {opt.label}
        </div>
      ))}
