...
const PaymentModal = ({
  ...
  return createPortal(
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 animate-in fade-in duration-300">
      <div className="absolute inset-0 bg-tinta/60 backdrop-blur-xl" role="presentation" aria-hidden="true" onClick={onClose}></div>
      <div className="bg-white rounded-[2.5rem] w-full max-w-md max-h-[90vh] shadow-[0_32px_64px_-12px_rgba(0,0,0,0.2)] relative z-[160] animate-in zoom-in-95 duration-200 border border-white overflow-hidden flex flex-col">
        ...
        <div className="bg-exito p-8 text-white relative shrink-0">
           ...
           <p className="text-[10px] font-black text-exito-suave uppercase tracking-widest mt-2 opacity-80">Orden de Compra #{paymentData.ordenId}</p>
        </div>
        ...
        <div className="p-8 space-y-6 overflow-y-auto scrollbar-premium">
          <div className="p-5 bg-slate-50/50 rounded-3xl border border-slate-100 flex justify-between items-center shadow-inner">
            ...
            <p className="text-2xl font-black text-peligro tracking-tighter">${(paymentData.total - paymentData.currentPaid).toLocaleString()}</p>
          </div>
          ...
          <div className="space-y-3 px-2">
            ...
            <div className="relative group">
              ...
              <input 
                ...
                className="w-full pl-10 p-4 bg-slate-50 border-2 border-slate-100 rounded-[1.5rem] text-lg font-black text-tinta outline-none focus:border-emerald-500 focus:bg-white transition-colors transition-shadow shadow-sm"
                placeholder="0"
              />
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3 pt-2">
            <button 
              ...
              disabled={isPaying}
              className="w-full py-4 bg-exito hover:bg-emerald-700 text-white rounded-[1.5rem] text-[10px] font-black uppercase tracking-[0.2em] shadow-xl transition-colors transition-shadow transition-transform active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2"
            >
            ...
            <button 
              ...
              disabled={isPaying}
              className="w-full py-4 bg-white border-2 border-slate-200 text-slate-600 hover:border-exito hover:text-exito rounded-[1.5rem] text-[10px] font-black uppercase tracking-[0.2em] transition-colors transition-transform active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2 font-bold"
            >
