...
const ProveedorGrid = ({
  ...
  if (loading) {
    return <div className="flex items-center justify-center h-40"><div className="animate-spin w-8 h-8 border-4 border-azul border-t-transparent rounded-full"></div></div>;
  }
        ...
        <div key={p.id_proveedor} className="bg-white p-6 rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 flex flex-col justify-between hover:shadow-2xl transition-shadow transition-transform hover:-translate-y-1">
          <div>
            <div className="flex justify-between items-start mb-4">
              <h3 className="text-xl font-black text-tinta tracking-tight uppercase leading-none">{p.nombre_empresa}</h3>
            ...
            <div className="mt-3 p-3 bg-rose-50 border border-peligro-suave rounded-xl">
              <p className="text-[9px] font-black text-rose-400 uppercase tracking-widest">Deuda Pendiente</p>
              <p className="text-sm font-black text-peligro">${(p.total_deuda || 0).toLocaleString()}</p>
            </div>
          </div>
          
          <button 
            onClick={() => onForecast(p)}
            className="mt-6 w-full py-3 bg-azul/10 text-azul hover:bg-azul-hondo hover:text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-colors transition-transform"
          >
            Calcular Pedido ⚡
          </button>
        </div>
