    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="bg-white w-full max-w-4xl max-h-[90vh] rounded-[2rem] shadow-2xl relative z-10 flex flex-col overflow-hidden animate-scale-in border border-slate-100">
        ...
        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
          <div>
            <h2 className="text-2xl font-black text-tinta tracking-tight uppercase">Orden a: {supplier.nombre_empresa}</h2>
        ...
        <div className="p-6 overflow-y-auto scrollbar-premium flex-1 space-y-6 bg-slate-50/50">
          ...
          {riskEval && (
            <div className={`p-4 rounded-2xl border-2 flex items-center justify-between ${riskEval.nivel === 'Bajo' ? 'bg-emerald-50 border-emerald-200' : (riskEval.nivel === 'Alto' ? 'bg-rose-50 border-rose-200' : 'bg-amber-50 border-amber-200')}`}>
              <div>
                <h4 className={`text-[10px] font-black uppercase tracking-widest ${riskEval.nivel === 'Bajo' ? 'text-exito' : 'text-peligro'}`}>
              ...
              <div className="text-right">
                ...
                <p className="text-xl font-black text-tinta">${riskEval.costo_total_estimado.toLocaleString()}</p>
              </div>
            </div>
          )}
                ...
                {(smartCart || forecastData).map(item => (
                  <tr key={item.id_producto} className={`border-b last:border-0 border-slate-50 transition-colors ${smartCart && !item.incluido ? 'opacity-40 grayscale' : 'hover:bg-slate-50/50'}`}>
                    ...
                    <td className="p-4 max-w-xs">
                      <p className="text-sm font-black text-tinta">{item.nombre}</p>
                      {item.razon_ia && item.incluido && <p className="text-[10px] leading-tight font-bold text-azul mt-1 italic">"{item.razon_ia}"</p>}
                    </td>
                    <td className="p-4">
                      <span className={`px-2 py-0.5 rounded text-[10px] font-black ${item.clasificacion_abc === 'A' ? 'bg-azul/10 text-azul' : 'bg-slate-100 text-slate-500'}`}>
                    ...
                    {smartCart && (
                      <td className="p-4 text-center font-black text-azul bg-azul/10">
                        {item.ajuste_ia}
                      </td>
                    )}
                    <td className="p-4 text-center border-x border-slate-50 bg-emerald-50/20">
                      {smartCart ? (
                        <input 
                          ...
                          className="w-20 bg-transparent text-center font-black text-exito text-lg border-b-2 border-emerald-200 focus:border-emerald-500 outline-none"
                        />
                      ) : (
                        <span className="font-black text-exito text-lg">{item.cantidad_sugerida}</span>
                      )}
                    </td>
                  </tr>
                ))}
          ...
          {!smartCart ? (
            <div className="flex gap-3 items-center">
              ...
              <button
                ...
                className="bg-azul hover:bg-azul-hondo text-white px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] shadow-lg transition-colors transition-shadow transition-transform active:scale-95 disabled:opacity-50 flex items-center gap-2"
              >
