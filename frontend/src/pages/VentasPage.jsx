    ...
    <div className="animate-fade-in pb-12 space-y-8 font-outfit">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-slate-100 pb-8">
        <div>
          ...
          <p className="text-slate-500 font-bold text-[10px] uppercase tracking-[0.2em] mt-1">Registro Transaccional Centralizado</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: 'Volumen Ingresos', value: `$${statsRender.totalVentas.toLocaleString('es-CO')}`, icon: '💰', color: 'text-emerald-500', bg: 'bg-emerald-50' },
          { label: 'Unidades Evacuadas', value: statsRender.totalProductos.toLocaleString('es-CO'), icon: '📦', color: 'text-indigo-600', bg: 'bg-indigo-50' },
          { label: 'Ticket Promedio', value: `$${statsRender.ventaPromedio.toLocaleString('es-CO', {maximumFractionDigits:0})}`, icon: '🧾', color: 'text-amber-500', bg: 'bg-amber-50' },
          { label: 'SKUs Movilizados', value: statsRender.productosUnicos, icon: '🔄', color: 'text-rose-500', bg: 'bg-rose-50' }
        ].map((stat, i) => (
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-900 text-[10px] font-black text-white uppercase tracking-[0.2em] whitespace-nowrap">
                ...
                <th className="p-6 text-right">P. Unitario</th>
                <th className="p-6 text-right">Total PxQ</th>
                <th className="p-6 text-center">Fecha Ejecución</th>
              </tr>
            </thead>
      ...
      {!filtroFecha && !filtroProducto && !filtroCategoria && totalVentasServer > PAGE_SIZE && (
        <div className="flex flex-col items-center justify-center gap-4 py-8">
          ...
          {currentOffset < totalVentasServer && (
            ...
            >
              {isLoading ? 'Sincronizando Archivos...' : 'Descargar Más Historial'}
            </button>
          )}
        </div>
      )}
          ...
          <div className="bg-white rounded-[3rem] w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl animate-scale-in overflow-hidden">
            <div className="flex justify-between items-center p-8 border-b border-slate-100 bg-white z-20 relative shrink-0">
              <div>
                 ...
                 <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mt-2">Líderes de Ventas Históricas</p>
              </div>
