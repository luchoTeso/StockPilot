    ...
    <div className="animate-fade-in p-2 md:p-6 pb-20 space-y-10 font-outfit">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-slate-100 pb-8">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h2 className="text-3xl font-black text-tinta tracking-tighter italic uppercase">Detalle de Productos</h2>
        ...
        <div className="bg-azul px-4 py-2 rounded-xl text-white font-black text-[9px] uppercase tracking-widest shadow-lg flex items-center gap-2">
      ...
      <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-2xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead>
              <tr className="bg-tinta text-[10px] font-black text-white uppercase tracking-[0.2em]">
              ...
              {loading ? (
                ...
                data.map((item, idx) => (
                  <tr key={idx} className="hover:bg-slate-50 transition-colors group">
                    <td className="p-6 font-black text-tinta uppercase tracking-tight">
                       <p className="text-sm">{item.nombre}</p>
                       <p className="text-[10px] text-slate-400 font-medium">#{item.id_producto}</p>
                    </td>
                    <td className="p-6">
                       <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase border ${item.category === 'A' ? 'bg-azul/10 text-azul border-azul/30' : item.category === 'B' ? 'bg-aviso-suave text-aviso border-amber-200' : 'bg-slate-100 text-slate-600 border-slate-200'}`}>{item.category === 'A' ? 'A (Alta)' : item.category === 'B' ? 'B (Media)' : 'C (Baja)'}</span>
                    </td>
                    <td className="p-6 text-center">
                       <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest text-white shadow-sm ${item.risk === 'high' ? 'bg-rose-500 animate-pulse' : item.risk === 'medium' ? 'bg-ambar' : 'bg-emerald-500'}`}>
                          {item.risk === 'high' ? 'ALTO' : item.risk === 'medium' ? 'MEDIO' : 'BAJO'}
                       </span>
                    </td>
                    <td className="p-6 text-center font-black text-azul italic">{(Number(item.velocity) || 0).toFixed(2)} <span className="text-[9px] opacity-50 not-italic">u/d</span></td>
                    <td className="p-6 text-center">
                       <p className="font-black text-tinta text-base tracking-tighter">{item.stock_actual}</p>
                       <p className="text-[9px] text-slate-400 uppercase font-black tracking-tighter mt-1">Sugerido: {item.stock_seguridad}u</p>
                    </td>
                    <td className="p-6 text-center">
                       <span className={`text-base font-black tracking-tighter ${item.days_to_exhaust === null || item.days_to_exhaust === Infinity || item.days_to_exhaust > 90 ? 'text-exito' : (item.days_to_exhaust < 5 ? 'text-peligro' : 'text-amber-600')}`}>
                          {item.days_to_exhaust === null || item.days_to_exhaust === Infinity ? 'Estable' : `${item.days_to_exhaust} días`}
                       </span>
                    </td>
                    <td className="p-6 text-right px-10 font-black text-tinta text-base tracking-tighter italic">${(Number(item.revenue) || 0).toLocaleString()}</td>
                  </tr>
                ))
              )}
            </tbody>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
         <div className="p-8 bg-white border border-slate-100 rounded-[2.5rem] shadow-xl group hover:scale-[1.02] transition-transform transition-shadow">
            ...
            <h4 className="text-[10px] font-black text-azul uppercase tracking-widest mb-3">Ritmo de Ventas</h4>
         ...
         <div className="p-8 bg-white border border-slate-100 rounded-[2.5rem] shadow-xl group hover:scale-[1.02] transition-transform transition-shadow">
            ...
            <h4 className="text-[10px] font-black text-peligro uppercase tracking-widest mb-3">Alertas de Agotamiento</h4>
