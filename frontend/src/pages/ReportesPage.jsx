...
const ReportesPage = () => {
  ...
  const getColorTipo = (tipo) => {
    const colores = {
      'Inventario': CHART.primary,
      'Ventas': CHART.positive,
      'Financiero': CHART.negative,
      'Operativo': CHART.secondary,
      'Otro': CHART.soft
    };
    return colores[tipo] || CHART.neutral;
  };
  ...
  return (
    <div className="animate-fade-in pb-12 font-outfit">
      ...
      <div className="bg-white/40 backdrop-blur-xl p-8 rounded-[2.5rem] border border-white/20 shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10 mt-2 animate-fade-in">
        <div>
          <h2 className="text-4xl font-black text-tinta tracking-tighter italic uppercase leading-none">Centro de <span className="text-azul">Reportes</span></h2>
          <p className="text-slate-500 font-bold text-[10px] uppercase tracking-[0.2em] mt-3 flex items-center gap-2">
            <span className="w-2 h-2 bg-azul rounded-full animate-pulse"></span>
        ...
        <div className="flex flex-wrap justify-end items-center gap-4">
           {isAdmin && (
             <button
               ...
               className="bg-emerald-500 hover:bg-exito text-white px-5 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 shadow-lg transition-all hover:-translate-y-1"
             >
           ...
           <div className="bg-white/60 px-6 py-3 rounded-2xl border border-white/40 shadow-sm">
              ...
              <span className="text-xl font-black text-tinta">{reportes.length} <span className="text-[10px] text-azul italic">docs</span></span>
           </div>
        </div>
      </div>
        ...
        {isAdmin && (
          ...
          <div className="bg-white rounded-[2.5rem] shadow-xl border border-slate-100 lg:sticky lg:top-6 max-h-[85vh] flex flex-col overflow-hidden">
            <div className="p-6 lg:p-8 overflow-y-auto scrollbar-hide flex-1">
              <h3 className="text-2xl font-black text-tinta tracking-tighter italic uppercase mb-6 pb-4 border-b border-slate-100 shrink-0">
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div>
                  ...
                  <input id="titulo" required type="text" value={formData.titulo} onChange={e => setFormData({...formData, titulo: e.target.value})} className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold focus:border-azul outline-none text-tinta" placeholder="Ej. Balance Mensual..."/>
                </div>
                ...
                {['Ventas', 'Financiero', 'Operativo', 'Inventario'].includes(formData.tipo) && (
                  <div className="bg-azul/5 p-4 border border-azul/30 rounded-[2rem] space-y-4">
                    <h4 className="text-[10px] font-black text-azul uppercase tracking-widest pl-1 mb-2 text-center flex items-center justify-center gap-1"><Clock size={12} /> Periodo Evaluado</h4>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <span className="block text-[10px] font-black text-azul uppercase tracking-widest pl-1 mb-1">Corte Inicial</span>
                        <CustomDatePicker 
                      ...
                      <div>
                        <span className="block text-[10px] font-black text-azul uppercase tracking-widest pl-1 mb-1">Corte Final</span>
                        <CustomDatePicker 
                ...
                <div>
                  ...
                  <input id="creador" required type="text" value={formData.creador} onChange={e => setFormData({...formData, creador: e.target.value})} className="w-full p-4 bg-slate-100 border border-slate-200 rounded-2xl text-sm font-bold focus:border-azul outline-none text-slate-500" readOnly/>
                </div>

                <div>
                  <label htmlFor="descripcion" className="block text-xs font-semibold text-slate-600 pl-1 mb-2">Conclusiones o Notas</label>
                  <textarea id="descripcion" required rows="3" value={formData.descripcion} onChange={e => setFormData({...formData, descripcion: e.target.value})} className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-medium focus:border-azul outline-none text-tinta resize-none hover:bg-white transition-colors" placeholder="Añade detalles relevantes o un resumen aquí..."></textarea>
                </div>
            ...
            <div className="mt-8 bg-white rounded-[2.5rem] p-8 shadow-xl border border-slate-100 relative overflow-hidden group">
               ...
               <div className="absolute -right-4 -top-4 w-32 h-32 bg-azul/10 rounded-full blur-3xl group-hover:bg-azul/10 transition-colors opacity-50"></div>
               
               <div className="relative z-10">
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-10 h-10 rounded-xl bg-azul/10 flex items-center justify-center shadow-sm border border-azul/30 text-azul">
                      <FileText size={20} />
                    </div>
                    <h4 className="text-tinta font-black text-lg tracking-tighter italic uppercase">Auditoría de Merma</h4>
                  </div>
                  ...
                  <button 
                    ...
                    className="w-full py-4 bg-azul text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-azul-hondo transition-colors transition-shadow transition-transform shadow-lg active:scale-95 flex items-center justify-center gap-3"
                  >
                  ...
                  <button 
                    ...
                    className="w-full py-3 mt-3 bg-white text-azul border-2 border-azul/30 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-azul/10 hover:border-azul transition-colors transition-transform active:scale-95 flex items-center justify-center gap-3"
                  >
             ...
             <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
               ...
               <div className="relative w-full sm:w-64">
                 ...
                 <input 
                   ...
                   className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold focus:border-azul outline-none text-tinta transition-colors"
                 />
               </div>
             </div>
          </div>
                    ...
                    filteredReportes.map(r => (
                      <tr key={r.id} className="hover:bg-slate-50 transition-colors group">
                        <td className="p-6 pl-8">
                          <strong className="block text-lg font-black text-tinta tracking-tighter mb-1">{r.titulo}</strong>
                        ...
                        <td className="p-6 text-tinta font-bold flex items-center gap-2 mt-4 lg:mt-0">
                        ...
                        <td className="p-6 pr-8">
                          <div className="flex justify-end gap-2">
                            <button onClick={() => descargarReporteExcel(r)} aria-label="Exportar a Excel" className="w-10 h-10 rounded-xl bg-emerald-50 text-exito hover:bg-exito hover:text-white border border-exito-suave flex items-center justify-center transition-colors transition-transform hover:scale-105 hover:-translate-y-1 shadow-sm" title="Exportar XLSX">
                              <Download size={18} />
                            </button>
                            {isAdmin && (
                              <>
                                <button onClick={() => handleEditClick(r)} aria-label="Editar Reporte" className="w-10 h-10 rounded-xl bg-amber-50 text-amber-600 hover:bg-ambar hover:text-white border border-aviso-suave flex items-center justify-center transition-colors transition-transform hover:scale-105 hover:-translate-y-1 shadow-sm" title="Editar Reporte">
                                  <Pencil size={18} />
                                </button>
                                <button onClick={() => eliminarReporte(r.id)} aria-label="Eliminar Reporte" className="w-10 h-10 rounded-xl bg-rose-50 text-peligro hover:bg-peligro hover:text-white border border-peligro-suave flex items-center justify-center transition-colors transition-transform hover:scale-105 hover:-translate-y-1 shadow-sm" title="Eliminar Reporte">
                                  <Trash2 size={18} />
                                </button>
                              </>
                            )}
