      ...
      <div className="space-y-10 animate-fade-in">
        ...
        <div className="border-b border-slate-100 pb-8">
          <div className="flex items-center gap-3">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-black text-tinta tracking-tighter italic uppercase">Mis Tiendas</h2>
            <span className="px-3 py-1 bg-azul/10 border border-azul/30 text-azul text-[10px] font-black uppercase tracking-wider rounded-xl">
        ...
        {tiendas.length === 0 ? (
           <div className="bg-white p-12 rounded-[2.5rem] text-center shadow-xl border border-slate-100 flex flex-col items-center justify-center gap-4">
            ...
            <h3 className="text-2xl font-black text-tinta tracking-tighter italic">Sin Sucursales</h3>
            ...
            <button
              ...
              className="mt-4 px-6 py-3.5 rounded-2xl bg-azul text-white text-[10px] font-black uppercase tracking-[0.2em] shadow-lg hover:bg-azul-hondo transition-all active:scale-95 flex items-center gap-2"
            >
          ...
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              {tiendas.map((t) => {
                ...
                return (
                  <div key={t.id_tienda} className={`bg-white rounded-[2rem] shadow-xl border ${isActiveSession ? 'border-azul ring-4 ring-azul/30' : 'border-slate-100'} p-6 sm:p-7 relative overflow-hidden group hover:shadow-2xl transition-all duration-300 flex flex-col justify-between`}>
                    {isActiveSession && (
                      <div className="absolute top-0 right-0 bg-azul text-white text-[9px] font-black uppercase tracking-widest px-4 py-1.5 rounded-bl-xl z-20 shadow-md">
                        Sesión Actual
                      </div>
                    )}
                    ...
                    <div>
                      <div className="flex items-start gap-4 mb-6 relative z-10">
                        ...
                        <div className="flex-1 min-w-0 pr-10">
                          <h3 className="text-xl font-black text-tinta tracking-tighter italic truncate" title={t.nombre_establecimiento}>{t.nombre_establecimiento}</h3>
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-3 relative z-10 pt-4 border-t border-slate-100">
                      {!isActiveSession && isStatusActive ? (
                        <button 
                          ...
                          className="w-full sm:flex-1 py-3 rounded-xl bg-tinta text-white text-[10px] font-black uppercase tracking-widest hover:bg-azul transition-colors flex items-center justify-center gap-2 shadow-sm"
                        >
                          Gestionar aquí <ArrowRight size={14} />
                        </button>
                      ) : (
                         <div className={`w-full sm:flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest text-center cursor-default ${isActiveSession ? 'bg-azul/10 text-azul font-black' : 'bg-slate-100 text-slate-400'}`}>
                      ...
                      <div className="flex items-center gap-2 w-full sm:w-auto">
                        <button 
                          ...
                          className="flex-1 sm:flex-none p-3 rounded-xl bg-slate-100 text-slate-500 hover:text-azul hover:bg-azul/10 transition-colors flex items-center justify-center"
                          title="Editar Tienda"
                        >
                          <Pencil size={16} />
                        </button>
                        <button 
                          ...
                          className={`flex-1 sm:flex-none p-3 rounded-xl transition-colors flex items-center justify-center ${isStatusActive ? 'bg-rose-50 text-rose-500 hover:bg-peligro-suave' : 'bg-emerald-50 text-emerald-500 hover:bg-exito-suave'}`}
              ...
              {tiendas.length === 1 && (
                <div className="bg-azul/5 border-2 border-dashed border-azul/20 rounded-[2rem] p-7 flex flex-col justify-between items-center text-center relative overflow-hidden group hover:border-azul hover:shadow-xl transition-all duration-300">
                  <div className="w-14 h-14 rounded-2xl bg-azul/10 text-azul flex items-center justify-center shadow-inner mb-3 group-hover:scale-110 transition-transform">
                    <Plus size={28} />
                  </div>
                  <div className="max-w-xs space-y-2">
                    <span className="inline-block px-3 py-1 bg-azul/10 text-azul rounded-full text-[9px] font-black uppercase tracking-widest">
                      Cupo 2 de 2 Disponible
                    </span>
                    <h4 className="text-xl font-black text-tinta tracking-tighter italic">Habilita una Segunda Sucursal</h4>
                  ...
                  <button
                    onClick={abrirModalCreate}
                    className="mt-5 w-full py-3.5 px-5 rounded-xl bg-azul hover:bg-azul-hondo text-white text-[10px] font-black uppercase tracking-[0.2em] shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2"
                  >
            ...
            <div className="pt-6 space-y-8">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-xl sm:text-2xl font-black text-tinta tracking-tighter italic uppercase">
                ...
                {tienda && (
                  <button
                    ...
                    className="self-start sm:self-auto flex items-center gap-2 px-4 py-2.5 rounded-xl bg-slate-100 text-slate-600 hover:text-azul hover:bg-azul/10 text-[10px] font-black uppercase tracking-widest transition-colors shadow-sm"
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/40">
                  <h4 className="flex items-center gap-2 text-tinta font-black text-xs uppercase tracking-widest border-b border-slate-100 pb-3 mb-4">
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <p className="text-base font-black text-tinta tracking-tight">{formatearFecha(tienda?.fecha_creacion || tienda?.anio_creacion)}</p>
                      </div>
                      <div>
                        <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Estado Operativo</p>
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider ${tienda?.estado === 'Activo' ? 'bg-emerald-50 text-exito' : 'bg-rose-50 text-peligro'}`}>
              ...
              <div className="bg-white p-6 sm:p-8 rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/40">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-5 mb-6">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-azul/10 text-azul flex items-center justify-center font-bold">
                      <Users size={20} />
                    </div>
                    <div>
                      <h4 className="text-lg font-black text-tinta tracking-tighter italic uppercase">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-5 py-3 rounded-xl bg-tinta hover:bg-azul text-white text-[10px] font-black uppercase tracking-widest shadow-md transition-colors self-start sm:self-auto active:scale-95"
                  >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {usuarios.map((u, i) => (
                      <div key={i} className="p-4 rounded-2xl bg-slate-50 border border-slate-100 hover:border-azul/30 hover:bg-white hover:shadow-md transition-all duration-200 flex items-center gap-3.5">
                        <div className={`w-11 h-11 rounded-xl flex items-center justify-center font-black text-base shadow-sm shrink-0 ${u.genero === 'Femenino' ? 'bg-peligro-suave text-peligro' : 'bg-azul/10 text-azul'}`}>
                          {(u.nombres || '?')[0].toUpperCase()}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-tinta text-sm font-black tracking-tight truncate" title={u.nombres}>{u.nombres}</p>
    ...
    if (!tienda) {
      return (
        <div className="bg-white p-12 rounded-[2.5rem] text-center shadow-xl border border-slate-100 flex flex-col items-center justify-center gap-4 animate-fade-in">
          <div className="mb-2 text-slate-400"><Store size={56} /></div>
          <h3 className="text-2xl font-black text-tinta tracking-tighter italic">Perfil Inexistente</h3>
    ...
    return (
      <div className="space-y-8 animate-fade-in">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-slate-100 pb-8 mb-8">
          <div>
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-black text-tinta tracking-tighter italic uppercase">Mi Tienda</h2>
        ...
        <div className="bg-white p-4 sm:p-6 lg:p-10 rounded-[2rem] sm:rounded-[2.5rem] shadow-xl border border-slate-100 relative overflow-hidden group hover:shadow-2xl transition-shadow duration-500">
          <div className="absolute top-0 right-0 w-64 h-64 bg-azul/10 rounded-full blur-3xl opacity-50 -translate-y-1/2 translate-x-1/3 group-hover:scale-110 transition-transform duration-700 pointer-events-none"></div>
          ...
          <div className="relative z-10">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-slate-100 pb-5 md:pb-8 mb-5 md:mb-8 gap-4 md:gap-6">
              <div className="flex items-center gap-5">
                <div className="w-20 h-20 bg-azul text-white rounded-3xl flex items-center justify-center shadow-lg shrink-0 transform -rotate-3 group-hover:rotate-0 transition-transform">
                  <Store size={36} />
                </div>
                <div>
                  <h3 className="text-xl sm:text-2xl lg:text-4xl font-black text-tinta tracking-tighter italic mb-1">
                    {tienda.nombre_establecimiento}
                  </h3>
                  <div className="flex items-center gap-3">
                    <span className="inline-flex items-center px-3 py-1 rounded-xl text-[10px] font-black uppercase tracking-widest border bg-emerald-50 text-exito border-exito-suave">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
              <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 hover:border-azul/30 hover:bg-azul/10 transition-colors">
                <h4 className="flex items-center gap-3 text-tinta font-black text-xs uppercase tracking-widest border-b border-white pb-3 mb-4">
              ...
              <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 flex flex-col items-center justify-center text-center gap-4 hover:border-azul/30 hover:bg-azul/10 transition-colors">
                ...
                <div>
                  ...
                  <p className="text-xl font-black text-azul tracking-tighter italic">{formatearFecha(tienda.fecha_creacion || tienda.anio_creacion)}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="pt-8">
          <h3 className="text-2xl font-black text-tinta tracking-tighter italic uppercase mb-6 pl-2 border-l-4 border-azul">
            Personal Registrado
          </h3>
          {usuarios.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {usuarios.map((u, i) => (
                <div key={i} className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/50 hover:-translate-y-1 hover:shadow-2xl hover:border-azul/30 transition-colors transition-shadow transition-transform duration-300 group flex items-start gap-5">
                  <div className="w-16 h-16 bg-azul/10 text-azul rounded-2xl flex items-center justify-center font-black text-2xl tracking-tighter border border-azul/30 shrink-0 group-hover:bg-azul-hondo group-hover:text-white transition-colors">
                     {(u.nombres || '?')[0].toUpperCase()}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-tinta text-lg font-black tracking-tighter truncate mb-1" title={u.nombres}>{u.nombres}</p>
  ...
  return (
    <div className="animate-fade-in pb-12 font-outfit">
      ...
      {(editModalOpen || createModalOpen) && (
        <div className="fixed inset-0 bg-tinta/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in font-outfit">
          <div className="bg-white rounded-[2.5rem] w-full max-w-[700px] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-scale-in">
            ...
            <div className="flex justify-between items-center p-6 lg:p-8 border-b border-slate-100 bg-slate-50/50">
              <div className="flex items-center gap-4">
                 ...
                 <div>
                   <h3 className="text-2xl font-black text-tinta tracking-tighter italic uppercase">
            ...
            <form onSubmit={submitTienda} className="overflow-y-auto p-6 lg:p-8 custom-scrollbar-hidden flex-1 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2 space-y-2">
                  ...
                  <input id="nombre_establecimiento" required name="nombre_establecimiento" value={formData.nombre_establecimiento} onChange={handleInputChange} type="text" className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold focus:border-azul outline-none text-tinta" placeholder="Ej. Tienda Central..." />
                </div>
                
                <div className="space-y-2">
                  <label htmlFor="razon_social" className="block text-xs font-semibold text-slate-600 pl-1">Razón Social</label>
                  <input id="razon_social" name="razon_social" value={formData.razon_social} onChange={handleInputChange} type="text" className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold focus:border-azul outline-none text-tinta" placeholder="Ej. Mi Tienda S.A.S" />
                </div>
                
                <div className="space-y-2">
                  <label htmlFor="documento" className="block text-xs font-semibold text-slate-600 pl-1">NIT / ID Tributario</label>
                  <input id="documento" name="documento" value={formData.documento} onChange={handleInputChange} type="text" className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold focus:border-azul outline-none text-tinta" placeholder="Ej. 900.123.456-7" />
                </div>

                <div className="md:col-span-2 pt-4 border-t border-slate-100">
                   <h4 className="text-[10px] font-black text-azul uppercase tracking-widest mb-4 flex items-center gap-1"><MapPin size={12} /> Datos de Ubicación</h4>
                </div>
                ...
                <div className="md:col-span-2 space-y-2">
                  ...
                  <input id="direccion" required name="direccion" value={formData.direccion} onChange={handleInputChange} type="text" className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold focus:border-azul outline-none text-tinta" placeholder="Avenida Siempre Viva 123..." />
                </div>

                <div className="space-y-2">
                  <label htmlFor="ciudad" className="block text-xs font-semibold text-slate-600 pl-1">Ciudad Sede</label>
                  <input id="ciudad" name="ciudad" value={formData.ciudad} onChange={handleInputChange} type="text" className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold focus:border-azul outline-none text-tinta" placeholder="Requerido para logística" />
                </div>

                <div className="space-y-2">
                  <label htmlFor="celular" className="block text-xs font-semibold text-slate-600 pl-1">Celular / Teléfono</label>
                  <input id="celular" name="celular" value={formData.celular} onChange={handleInputChange} type="tel" className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold focus:border-azul outline-none text-tinta" placeholder="+57 300 000 0000" />
                </div>
              ...
              <div className="flex flex-col sm:flex-row justify-end gap-3 pt-8 pb-2">
                ...
                <button type="submit" disabled={editLoading} className="px-8 py-4 rounded-2xl bg-azul text-white text-[10px] font-black uppercase tracking-[0.2em] flex items-center justify-center gap-2 shadow-lg hover:bg-azul-hondo transition-colors transition-shadow transition-transform active:scale-95 disabled:opacity-50">
